...
namespace Application
{
    ...
    {
        #region Instancias
        //private UserEntity ObjUserEntity = new UserEntity();
                ...
                case 0:
                    try
                    {
                        ...
                        if (alert)
                        {
                            ...
                            GridSearch.DataSource = ObjBusiness.Search(Convert.ToByte(comboTypeSearch.SelectedIndex), Valor);
                            for (int i = 0; i < GridSearch.Rows.Count; i++)
                                GridSearch.Rows[i].Cells["hora"].Value = GridSearch.Rows[i].Cells["hora"].Value.ToString().Substring(0, 7);
                        }
                        else
                        {
                            Alerts pregunta = new Alerts((byte)TypeIcon.Warning, "Búsqueda vacia", "No se permiten búsquedas vacias", "Por favor, ingrese un rato poder continuar la búsqueda.", false);
