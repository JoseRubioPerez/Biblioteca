...
namespace Application
{
    public partial class ChangeMyPassword : FatherConfig
    {
        ...
        {
            Alerts pregunta;
            pregunta = new Alerts((byte)TypeIcon.Warning, "Confirmación", "¿Estas seguro de que deaseas cambiar tu contraseña?", "Sí aceptas, tu contraseña será cambiada por la que acabas de ingresar.", true);
            dr = pregunta.ShowDialog();
            if (dr == DialogResult.Yes)
            {
                ObjBusiness.ChangePassword(Admin[0], Admin[1], txtNewPassword.Text.Trim());
                Limpiar();
                pregunta = new Alerts((byte)TypeIcon.Info, "Tarea realizada", "Cambio de contraseña", "La contraseña se ha cambiado exitosamente.\nPara asegurar los cambios, por favor de cerrar sesión\ncon su anterior contraseña e iniciar con la nueva contraseña.", false);
                pregunta.Show();
            }
            dr = DialogResult.No;
        }
        ...
        private void EventoClick(object sender, EventArgs e)
        {
            ...
            switch (menu)
            {
                case 0:
                    ...
                    {
                        Alerts pregunta = new Alerts((byte)TypeIcon.Warning, "Contraseñas diferentes", "Verifique su información", "La contreaseña no coincide con su confirmación, verifique que ambas sean iguales", false);
                        pregunta.Show();
                    }
                    break;
