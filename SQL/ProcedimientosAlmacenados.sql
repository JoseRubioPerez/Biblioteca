...
CREATE PROCEDURE SearchMoves @p1 VARCHAR(20) --Apellido Paterno
	,@p2 VARCHAR(20) --Apellido Materno
	,@p3 TINYINT --Index Area Inicio
	,@p4 TINYINT --Index Area Fin
	,@p5 CHAR(1) --Sexo Inicio
	,@p6 CHAR(1) --Sexo Fin
	,@p7 VARCHAR(10) -- Fecha de Inicio
	,@p8 VARCHAR(10) -- Fecha de Fin
AS
BEGIN
	IF @p5 = 'A'
		OR @p6 = 'A'
	...
	BEGIN
		...
		BEGIN
			...
			INNER JOIN dbo.Usuarios AS b ON b.nc = a.nc
			INNER JOIN dbo.Areas AS c ON c.id = b.area
			WHERE a.fecha BETWEEN CAST(@p7 AS DATE)
					AND CAST(@p8 AS DATE)
				AND (
					c.id BETWEEN @p3
						AND @p4
